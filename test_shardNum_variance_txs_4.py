...
# 创建一个5行2列的子图布局
fig, axs = plt.subplots(nrows=7, ncols=2, figsize=(30, 20), sharex=True)
...
for idx, txs in enumerate(all_txs_values):
    ...
    ax2.set_ylabel('ctxs Reduction Ratio')
    ax2.set_xlabel('Number of Shards')
    ax2.legend()
    ax2.grid(True)

# 设置全局标题
fig.suptitle('Variance Comparison and Cross-Shard txs Reduction Ratio with Different all_txs Values')
