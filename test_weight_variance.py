...
clpa_sharding = {}
cpla_plus_sharding = {}
integer_range = range(11)
float_range = [i / 10.0 for i in integer_range]
for v in float_range:
    ...
    for idx, txs in enumerate(all_txs_values):
        ...
        ax.plot(sorted(clpa_sharding.keys()),
                [clpa_sharding[k][txs]['variance'] for k in sorted(clpa_sharding.keys())],
                label='clpa_sharding')
        # 绘制曲线
        ax.plot(sorted(cpla_plus_sharding.keys()),
                [cpla_plus_sharding[k][txs]['variance'] for k in sorted(cpla_plus_sharding.keys())],
                label='current')
    ...
    axs[len(all_txs_values) - 1].set_ylabel('variance')
